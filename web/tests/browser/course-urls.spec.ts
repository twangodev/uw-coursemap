...
test("canonical course URLs render and cross-list aliases redirect without supporting raw IDs", async ({
  ...
  for (const alias of ["CS300", "compsci-300", "cs-300"]) {
    const response = await request.get(`/courses/${alias}`, {
      maxRedirects: 0,
    });
    expect(response.status()).toBe(308);
    expect(response.headers().location).toBe("/courses/COMPSCI_300");
  }
  const crosslist = await request.get("/courses/ece-759", { maxRedirects: 0 });
  expect(crosslist.status()).toBe(308);
  expect(crosslist.headers().location).toBe("/courses/COMPSCI_ECE_EMA_EP_ME_759");
  const response = await page.goto("/courses/COMPSCI_300");
...
test("course lists link directly to readable pages", async ({ page }) => {
  ...
  await expect(link).toHaveAttribute("href", "/courses/COMPSCI_300");
  await link.click();
  await expect(page).toHaveURL(/\/courses\/COMPSCI_300$/);
});
