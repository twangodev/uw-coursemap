...
import {type Course, sanitizeCourseToReferenceString} from "$lib/types/course.ts";
...
export const load = async ({ fetch }) => {
    ...
    const courseReferences = quickStatistics.top_100_a_rate_chances.map((item) => item.reference);
    const courses: Course[] = await Promise.all(courseReferences.map(async (reference) => {
        const sanitized = sanitizeCourseToReferenceString(reference);
        const response = await fetch(`${PUBLIC_API_URL}/course/${sanitized}.json`)
