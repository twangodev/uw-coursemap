...
export const load = async ({ params, fetch }) => {
    ...
    const elementDefinitionsResponse = await fetch(`${PUBLIC_API_URL}/graphs/${subject}.json`)
    ...
    const styleEntriesResponse = await fetch(`${PUBLIC_API_URL}/styles/${subject}.json`)
