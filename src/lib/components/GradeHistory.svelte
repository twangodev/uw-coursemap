...
{#if rows.length}
  <section class="history-charts" aria-label="Historical grade outcomes">
    <div>
      ...
      <div class="mix-legend">{#each series as item}<span><i style:background={item.color}></i>{item.label}</span>{/each}</div>
    ...
    <div>
      ...
      <BarChart
        ...
        y="total"
        series={[
          { key: "total", label: "Recorded outcomes", color: "var(--accent)" },
        ]}
        height={240}
        props={{
...
<style>
  .mix-legend { display: flex; gap: 12px; flex-wrap: wrap; margin-bottom: 16px; font-size: 11px; color: var(--muted); }
  .mix-legend span { display: inline-flex; align-items: center; gap: 5px; }
  .mix-legend i { width: 7px; height: 7px; border-radius: 2px; }
  .history-charts {
    display: grid;
