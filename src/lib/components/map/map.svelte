<script lang="ts">
	...
	let selectedJSDate = $state(initialDate);
	...
	async function loadDataForDate(date: Date) {
		// Use provided URL directly if it doesn't contain date placeholder
		let dataUrl = buildingHighlightsUrl;
		
		// If the URL contains date placeholder, replace it
		if (buildingHighlightsUrl.includes('{date}')) {
			const dateStr = formatDateForAPI(date);
			dataUrl = buildingHighlightsUrl.replace('{date}', dateStr);
		}
	...
	onMount(async () => {
		...
		function render(time: number, tripTime: number) {
			...
			if (showTrips && tripsData) {
				...
				layers.unshift(trips);
			}

			// Add building highlights layer
		...
		function loop() {
			if (!showTrips || !maxTimestamp) return;
