...
def build_from_pagination(course_ref_to_course, terms, url, madgrades_api_key, stats, logger: Logger):
    if url is None:
        return
    ...
    for madgrade_course in results:
        ...
        madgrades_data = MadgradesData.from_madgrades(terms, grades_url, madgrades_api_key)
    ...
    build_from_pagination(course_ref_to_course, terms, next_page_url, madgrades_api_key, stats, logger)
...
def add_madgrades_data(course_ref_to_course, madgrades_api_key, stats, logger: Logger):
    ...
    build_from_pagination(course_ref_to_course, terms, madgrades_api_endpoint + "courses", madgrades_api_key, stats, logger)
