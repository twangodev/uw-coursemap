...
def get_embedding(client: OpenAI, model, text):
...
def find_best_prerequisite(client, model, course, prerequisites, max_prerequisites) -> list[Course]:
    course_embedding = get_embedding(client, model, course.get_full_summary())
    prerequisite_embeddings = [
        (prereq, get_embedding(client, model, prereq.get_short_summary())) for prereq in prerequisites
    ]
...
def prune_prerequisites(client, model, course: Course, course_ref_to_course, max_prerequisites, logger: Logger):
...
async def optimize_prerequisite(course, client, model, course_ref_to_course, max_prerequisites, max_retries, logger):
    retries = 0
    while retries < max_retries:
        try:
            prune_prerequisites(client, model, course, course_ref_to_course, max_prerequisites, logger)
            return
...
async def optimize_prerequisites(
    ...
    async def optimize_course(course):
        ...
        async with semaphore:
            await optimize_prerequisite(course, client, model, course_ref_to_course, max_prerequisites, max_retries, logger)
            completed_courses += 1
