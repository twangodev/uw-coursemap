...
For student-focused previews, run `enrich` with `--task inference/tasks/student_summary.json` and `--reuse-job` for the selected catalog enrichment jobs. The workflow uses the snapshot's current enrollment roster, summarizes each professor independently, and keeps former instructors in a historical paragraph. Grade sentences are computed temporarily from raw letter-grade counts for the last three usable terms; no aggregate-stat table is exported. Review and grade citations identify their source rows. Results appear in `llm_student_summary_json`; existing catalog metadata and prerequisite trees are retained.
