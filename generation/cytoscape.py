...
def get_subgraphs(
    ...
    for reference in optimized_prerequisites:
        if reference not in course_ref_to_course:
            ...
            continue
        edge = create_edge(
            target=course.get_identifier(), source=reference.get_identifier()
        )
        to_add.add(edge)
        c: Course = course_ref_to_course[reference]
        to_add.add(
            get_subgraphs(
                c, course_ref_to_course, seen, global_graph, subject_graph, course_graph
            )
        )
...
def build_graphs(
    course_ref_to_course: dict[Course.Reference, Course],
    subject_to_courses: dict[str, set[Course]],
):
    ...
    for subject, course_set in tqdm(
        subject_to_courses.items(), desc="Building graphs", unit="subject"
    ):
        for course in tqdm(
            course_set, desc=f"Building graphs for {subject}", unit="course"
        ):
            ...
            get_subgraphs(
                ...
                set(),
                graph,
                subject_to_graph[subject],
                course_to_graph[course.get_identifier()],
            )
...
def generate_style_from_graph(graph, color_map):
    ...
    return [
        generate_style(parent, color) for parent, color in zip(set(parents), colors)
    ]


def generate_styles(subject_to_graph, color_map):
    return {
        subject: generate_style_from_graph(subject_to_graph[subject], color_map)
        for subject in subject_to_graph
    }
