...
UW Course Map has two maintained components:

1. **Frontend**: SvelteKit 5 application using configurable external API endpoints.
2. **Data pipeline**: Manual local scraping and LLM enrichment, publishing Parquet to HF.

The Flask/Elasticsearch backend is retired. A Parquet-backed Cloudflare Worker is planned.
...
- **Search Client**: Existing frontend API calls remain for the upcoming Worker migration
